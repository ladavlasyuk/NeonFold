...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private var didSaveColdLaunchPushFromOptions = false
    private var foregroundPushEnrichmentIDs = Set<String>()
    private let richPushIdentifierPrefix = "nf-rich-"
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        observeLaunchState()
        LaunchState.shared.discardPersistedPushDestination()
    ...
    func handleSceneNotificationResponse(_ response: UNNotificationResponse) {
        if didSaveColdLaunchPushFromOptions {
            didSaveColdLaunchPushFromOptions = false
            return
        }
    ...
    private func scheduleLaunchLoaderFinish() {
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if !self.didReceiveConfigResponse
                ...
                && !LaunchState.shared.isPushSessionBlockingConfigPresentation()
                && !LaunchState.shared.didCommitLaunchDestination {
                if LaunchState.shared.openLastSuccessfulDestination() {
    ...
    private func updateLaunchPresentation() {
        ...
        if isLaunchLoaderActive {
            ...
            if let destination = state.browserDestination {
                logTiming("Launch decision: browser — switching loader")
                isLaunchLoaderActive = false
                expectsBrowserDestination = false
                cancelLoaderSafetyTimeout()
                OrientationController.shared.unlockAllOrientations()
                let forceReload = LaunchState.shared.consumeForceBrowserReload()
                switchRootToBrowser(
                    window: window,
                    destination: destination,
                    forceReload: forceReload,
                    asNewPage: forceReload && LaunchState.shared.activePushAddress == destination
                )
                return
            }
            ...
            if expectsBrowserDestination || state.pendingDestination != nil {
                logTiming("Launch: browser decision pending — keeping loader")
                return
            }

            if didReceiveConfigResponse {
                logTiming("Launch decision: native — switching loader")
                isLaunchLoaderActive = false
                cancelLoaderSafetyTimeout()
    ...
    private func switchRootToBrowser(window: UIWindow, destination: String, forceReload: Bool = false, asNewPage: Bool = false) {
        OrientationController.shared.unlockAllOrientations()
        ...
        if let existing = rootBrowser {
            if (forceReload || existing.destination != destination), let address = URL(string: destination) {
                ...
                if asNewPage {
                    existing.openAsNewPage(address: address)
                } else {
    ...
    private func presentBrowserDestination(_ destination: String, forceReload: Bool = false, asNewPage: Bool = false) {
        guard let window = window else { return }

        if let browser = window.rootViewController as? ContentBrowserController {
            rootBrowser = browser
            ContentPresenter.shared.present(destination: destination, forceReload: forceReload, asNewPage: asNewPage)
            return
        }
        ...
        if window.rootViewController?.presentedViewController != nil {
            window.rootViewController?.dismiss(animated: false) { [weak self] in
                ...
                self.switchRootToBrowser(window: window, destination: destination, forceReload: forceReload, asNewPage: asNewPage)
            }
        } else {
            switchRootToBrowser(window: window, destination: destination, forceReload: forceReload, asNewPage: asNewPage)
        }
    }
    ...
    private func observeLaunchState() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handlePushOpenRequested(_:)),
            name: LaunchState.pushOpenRequestedNotification,
            object: nil
        )
    }

    @objc private func handlePushOpenRequested(_ notification: Notification) {
        guard let address = notification.userInfo?["address"] as? String else { return }
        presentBrowserDestination(address, forceReload: true, asNewPage: true)
    }
    ...
    private func continueLaunchFlow() {
        ...
        if LaunchState.shared.activePushAddress != nil {
            print("[AFSDK] Push destination is active for this process. Config will not replace it.")
            if LaunchState.shared.isStoredDestinationExpired() {
                refreshConfigInBackground(applyDestinationToCurrentSession: false)
            } else {
                print("[AFSDK] Stored destination is still valid. Skipping config request.")
            }
            updateLaunchPresentation()
            return
        }

        if LaunchState.shared.openUnexpiredStoredDestination() {
            print("[AFSDK] Stored destination is still valid. Opening it without a config request.")
            didReceiveConfigResponse = true
        ...
        if let storedPayload = LaunchState.shared.storedConfigPayload() {
            print("[AFSDK] Waiting for config before opening a destination.")
    ...
    private func refreshConfigInBackground(applyDestinationToCurrentSession: Bool) {
        if let storedPayload = LaunchState.shared.storedConfigPayload() {
    ...
    private func handleRemoteNotificationUserInfo(_ userInfo: [AnyHashable: Any], fromUserTap: Bool) {
        ...
        guard !LaunchState.shared.isPermanentNativeFlow() else {
            print("[Push] Tap ignored — native flow is active")
            return
        }
        guard let addressString = extractPushAddress(from: userInfo) else {
            print("[Push] Tap ignored — no address in payload")
            return
        }

        print("[Push] Opening push destination: \(addressString)")
        DispatchQueue.main.async {
            _ = LaunchState.shared.acceptPushAddress(addressString)
        }
    }
    ...
    private func handleLaunchPushNotification(_ userInfo: [AnyHashable: Any]) {
        ...
        guard let addressString = extractPushAddress(from: userInfo) else {
            print("[Push] Cold launch push ignored — no address in payload")
            return
        }

        if LaunchState.shared.acceptPushAddress(addressString) {
            didSaveColdLaunchPushFromOptions = true
            print("[Push] Cold launch push destination accepted for this process")
        }
    }
    ...
    private func sendMergedPayload(_ payload: [String: Any], applyDestinationToCurrentSession: Bool = true) {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let error = error {
                ...
                print("[AFSDK] No server response — opening last successful destination when available.")
                DispatchQueue.main.async {
    ...
    private func handleMergedPayloadResponse(
        ...
        guard let data = data,
            ...
            DispatchQueue.main.async {
                guard LaunchState.shared.browserDestination == nil,
                      !LaunchState.shared.isPushSessionBlockingConfigPresentation() else { return }
                if LaunchState.shared.openLastSuccessfulDestination() {
                    print("[AFSDK] Opened last successful destination.")
                    return
                }
